...
const MESSAGES: Record<Lang, Record<ImageErrorType, string>> = {
  ko: {
    ...
    "decode-failed": "이미지를 불러올 수 없습니다. 다른 파일을 선택해 주세요.",
    ...
    "output-too-large":
      "이 크기의 이미지는 현재 브라우저에서 처리할 수 없습니다. 출력 크기를 줄여 주세요.",
    "process-failed": "이미지를 처리하지 못했습니다. 설정을 확인하고 다시 시도해 주세요.",
  },
  en: {
    "unsupported-format": "Only PNG, JPG, and WebP images are supported.",
    "decode-failed": "This image could not be loaded. Please choose another file.",
    ...
    "output-too-large":
      "This image size cannot be processed in your browser. Reduce the output dimensions and try again.",
    "process-failed": "The image could not be processed. Check the settings and try again.",
  },
};
 ...
 * fallback 은 원인을 알 수 없을 때 쓰는 값이다: 최적화는 "encode-failed",
 * 리사이즈·크롭은 "process-failed" 처럼 도구가 쓰는 문구에 맞춘다.
 */
export function normalizeImageError(
  err: unknown,
  fallback: ImageErrorType = "encode-failed",
): ImageErrorType {
