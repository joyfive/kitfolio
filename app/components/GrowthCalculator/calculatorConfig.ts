...
export interface ModeConfig {
  /** 이 모드가 속한 통합 페이지의 registry slug. PageHead·ToolGuide·Faq·RelatedTools가
   *  전부 이 slug로 콘텐츠를 가져오므로, 같은 페이지에 묶인 모드는 값이 같아야 한다. */
  slug: string;
  tabs: TabDef[];
  inputs: InputDef[];
  outputs: OutputDef[];
...
// 2026-09: 단기 1회성 변화(성장률·퍼센트 차이·목표 성장률·필요 증가량·역산) 5종을
// growth-rate-calculator 한 페이지(?mode=)로 통합.
const TABS_GROWTH: TabDef[] = [
  { mode: "growth-rate",        label: { ko: "성장률",      en: "Growth Rate" } },
  { mode: "percent-difference", label: { ko: "퍼센트 차이", en: "% Difference" } },
  { mode: "goal-growth",        label: { ko: "목표 성장률", en: "Goal Growth" } },
  { mode: "required-growth",    label: { ko: "필요 증가량", en: "Required Growth" } },
  { mode: "reverse-growth",     label: { ko: "역산",        en: "Reverse Growth" } },
];

// 다기간 복리(CAGR·복리 성장·미래값 예측) 2종은 cagr-calculator 한 페이지(?mode=)로 통합.
const TABS_CAGR: TabDef[] = [
  { mode: "cagr",            label: { ko: "CAGR",     en: "CAGR" } },
  { mode: "compound-growth", label: { ko: "복리 성장", en: "Compound" } },
];
